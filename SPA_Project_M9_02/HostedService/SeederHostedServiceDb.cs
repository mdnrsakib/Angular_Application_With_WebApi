...
namespace SPA_Project_M9_02.HostedService
{
    public class SeederHostedServiceDb : IHostedService
    {
        ...
        public async Task SeedDbAsync(TravelTourDbContext db)
        {
            ...
            if (!db.TravelAgents.Any())
            {
                ...
                var f1 = new PackageFeature { TransportMode = TransPortMode.Bus, HotelBooking = "Chattrogram View", Status = true,TourPackages=p2 };
                await db.PackageFeatures.AddAsync(f1);
                var f2 = new PackageFeature { TransportMode = TransPortMode.CruiseShip, HotelBooking = "Cox's View", Status = true,TourPackages=p1 };
                await db.PackageFeatures.AddAsync(f2);
